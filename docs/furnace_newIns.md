# new Furnace instrument format

Source: https://github.com/tildearrow/furnace/blob/master/papers/newIns.md
Fetched: 2026-04-16

The aim of this format is to greatly reduce the size of instrument files by only storing used parameters ("featural" format).

# information

- Features which are not recognized by Furnace will be ignored.
- Instruments are not compressed using zlib, unlike Furnace songs.
- All numbers are little-endian.
- `STR` is a UTF-8 zero-terminated string.
- `f` indicates a floating point number.

# header

.fui files use the following header:

```
size | description
-----|------------------------------------
  4  | "FINS" format magic
  2  | format version
  2  | instrument type
 ??? | features...
```

instruments in a .fur file use the following header instead:

```
size | description
-----|------------------------------------
  4  | "INS2" block ID
  4  | size of this block
  2  | format version
  2  | instrument type
 ??? | features...
```

a feature uses the following format:

```
size | description
-----|------------------------------------
  2  | feature code
  2  | length of block
 ??? | data...
```

## instrument types

- 0: SN76489
- 1: FM (OPN)
- 2: Game Boy
- 3: C64
- 4: Amiga/sample
- **5: PC Engine**
- 6: AY-3-8910
- 7: AY8930
- 8: TIA
- 9: SAA1099
- 10: VIC
- 11: PET
- 12: VRC6
- 13: OPLL
- 14: OPL
- 15: FDS
- 16: Virtual Boy
- 17: Namco 163
- 18: SCC
- 19: OPZ
- 20: POKEY
- 21: PC Speaker
- 22: WonderSwan
- 23: Lynx
- 24: VERA
- 25: X1-010
- 26: VRC6 (saw)
- 27: ES5506
- 28: MultiPCM
- 29: SNES
- 30: Sound Unit
- 31: Namco WSG
- 32: OPL (drums)
- 33: FM (OPM)
- 34: NES
- 35: MSM6258
- 36: MSM6295
- 37: ADPCM-A
- 38: ADPCM-B
- 39: SegaPCM
- 40: QSound
- 41: YMZ280B
- 42: RF5C68
- 43: MSM5232
- 44: T6W28
- 45: K007232
- 46: GA20
- 47: Pokemon Mini/QuadTone
- 48: SM8521
- 49: PV-1000
- 50: K053260
- 52: TED
- 53: C140
- 54: C219
- 55: ESFM
- 56: PowerNoise (noise)
- 57: PowerNoise (slope)
- 58: Dave
- 59: NDS
- 60: GBA DMA
- 61: GBA MinMod
- 62: Bifurcator
- 63: SID2
- 64: Supervision
- 65: uPD1771C
- 66: SID3

## feature codes

- `NA`: instrument name
- `FM`: FM ins data
- `MA`: macro data
- `64`: C64 ins data
- `GB`: Game Boy ins data
- `SM`: sample ins data
- `O1`: operator 1 macros
- `O2`: operator 2 macros
- `O3`: operator 3 macros
- `O4`: operator 4 macros
- `LD`: OPL drums mode data
- `SN`: SNES ins data
- `N1`: Namco 163 ins data
- `FD`: FDS/Virtual Boy ins data
- `WS`: wavetable synth data
- `SL`: list of samples (<233)
- `WL`: list of wavetables (<233)
- `LS`: list of samples (>=233)
- `LW`: list of wavetables (>=233)
- `MP`: MultiPCM ins data
- `SU`: Sound Unit ins data
- `ES`: ES5506 ins data
- `X1`: X1-010 ins data
- `NE`: NES DPCM sample map data
- `EF`: ESFM ins data
- `PN`: PowerNoise ins data
- `S2`: SID2 ins data
- `S3`: SID3 ins data
- `EN`: end of features
  - if you find this feature code, stop reading the instrument.
  - instruments in a .fur shall end with this feature code.

# instrument name (NA)

```
size | description
-----|------------------------------------
 STR | instrument name
```

# FM data (FM)

FM operator order is:
- 1/3/2/4 (internal order) for OPN, OPM, OPZ and OPL 4-op
- 1/2/?/? (? = unused) for OPL 2-op and OPLL

```
size | description
-----|------------------------------------
  1  | flags
     | - bit 4-7: op enabled
     |   - op order from 4 to 7: 0, 2, 1, 3
     |   - 2-op instruments: 0, 1, x, x
     | - bit 0-3: op count
-----|------------------------------------
     | **base data**
     | /7 6 5 4 3 2 1 0|
  1  | |x| ALG |x| FB  |
  1  | |FMS2 |AMS| FMS |
  1  | |AM2|4| LLPatch |
  1  | |xxxxxxx| Block | (>=224)
-----|------------------------------------
     | **operator data x opCount**
     | /7 6 5 4 3 2 1 0|
  1  | |r| D T |  MULT |  (r = KSR)
  1  | |s|     T L     |  (s = SUS)
  1  | |R S|v|   A R   |  (v = VIB)
  1  | |A|KSL|   D R   |  (A = AM)
  1  | |e|KVS|   D2R   |  (e = EGT)
  1  | |  S L  |  R R  |
  1  | |  DVB  |  SSG  |
  1  | | DAM |DT2| W S |
```

# macro data (MA)

notes:
- the macro range varies depending on the instrument type.
- "macro open" indicates whether the macro is collapsed or not in the instrument editor.
- meaning of extended macros varies depending on instrument type.
- meaning of panning macros varies depending on instrument type:
  - for hard-panned chips (e.g. FM and Game Boy): left panning is 2-bit panning macro (left/right)
  - otherwise both left and right panning macros are used
- ADSR/LFO macros before version 245 have different behavior (see sub-section below).

```
size | description
-----|------------------------------------
  2  | length of macro header
 ??? | data...
```

each macro is represented like this:

```
size | description
-----|------------------------------------
  1  | macro code
     | - 0: vol
     | - 1: arp
     | - 2: duty
     | - 3: wave
     | - 4: pitch
     | - 5: ex1
     | - 6: ex2
     | - 7: ex3
     | - 8: alg
     | - 9: fb
     | - 10: fms
     | - 11: ams
     | - 12: panL
     | - 13: panR
     | - 14: phaseReset
     | - 15: ex4
     | - 16: ex5
     | - 17: ex6
     | - 18: ex7
     | - 19: ex8
     | - 20: ex9
     | - 21: ex10
     | - 255: stop reading and move on
  1  | macro length
  1  | macro loop
  1  | macro release
  1  | macro mode
  1  | macro open/type/word size
     | - bit 6-7: word size
     |   - 0: 8-bit unsigned
     |   - 1: 8-bit signed
     |   - 2: 16-bit signed
     |   - 3: 32-bit signed
     | - bit 3: instant release (>=182)
     | - bit 1-2: type
     |   - 0: normal (sequence)
     |   - 1: ADSR
     |   - 2: LFO
     | - bit 0: open
  1  | macro delay
  1  | macro speed
 ??? | macro data
     | - length: macro length x word size
```

**CRITICAL FOR PC ENGINE**: Only vol (0), arp (1), and wave (3) macros are relevant. Noise (ex1=5) may also appear. Code 255 terminates the macro list.

## word size interpretation

- 0 (8-bit unsigned): read 1 byte per entry, unsigned
- 1 (8-bit signed): read 1 byte per entry, signed
- 2 (16-bit signed): read 2 bytes per entry, signed (LE)
- 3 (32-bit signed): read 4 bytes per entry, signed (LE)

## interpreting macro mode values

- sequence (normal): standard step-through envelope
- ADSR:
  - val[0]: bottom
  - val[1]: top
  - val[2]: attack
  - val[3]: hold time
  - val[4]: decay
  - val[5]: sustain level
  - val[6]: sustain hold time
  - val[7]: decay 2
  - val[8]: release
- LFO:
  - val[11]: speed
  - val[12]: waveform (0=triangle, 1=saw, 2=pulse)
  - val[13]: phase
  - val[14]: loop
  - val[15]: global

## ADSR/LFO macro behavior

ADSR/LFO macros work by altering a fixed-point accumulator with 8-bit fractional precision.

ADSR mode: accumulator starts at bottom, increases by attack rate, then decreases by decay rate until sustain level, etc.

LFO mode: depends on shape (saw/triangle/pulse). Speed is fixed-point, phase is 0-1023.

### ADSR/LFO macro behavior before version 245

In previous versions, ADSR used integer accumulator 0-255, scaled between bottom and top. LFO used integer accumulator 0-1023.

Conversion formulas:
1. If bottom > top (inverted range), set bottom to `top+((255+(bottom-top)*255)/256)`
2. Range = abs(top-bottom)
3. For ADSR: rates = (rate*range*256)/255; sustain = bottom+(((top-bottom)*susLevel)/255)
4. For LFO speed: triangle=(range*speed)/2, saw=(range*speed)/4, square=speed*64

# sample ins data (SM)

```
size | description
-----|------------------------------------
  2  | initial sample
  1  | flags
     | - bit 2: use wave
     | - bit 1: use sample
     | - bit 0: use sample map
  1  | waveform length
 4?? | sample map... (120 entries, only read if sample map is enabled)
```

sample map format:
```
  2  | note to play (>=152) or reserved
  2  | sample to play
```

# wavetable synth data (WS)

```
size | description
-----|------------------------------------
  4  | first wave
  4  | second wave
  1  | rate divider
  1  | effect
     | - bit 7: single or dual effect
  1  | enabled
  1  | global
  1  | speed (+1)
  1  | parameter 1
  1  | parameter 2
  1  | parameter 3
  1  | parameter 4
```

# operator macro data (O1, O2, O3 and O4)

similar to macro data, but using these macro codes:

- 0: AM
- 1: AR
- 2: DR
- 3: MULT
- 4: RR
- 5: SL
- 6: TL
- 7: DT2
- 8: RS
- 9: DT
- 10: D2R
- 11: SSG-EG
- 12: DAM
- 13: DVB
- 14: EGT
- 15: KSL
- 16: SUS
- 17: VIB
- 18: WS
- 19: KSR

# C64 data (64)

```
size | description
-----|------------------------------------
  1  | flags 1
     | - bit 7: dutyIsAbs
     | - bit 6: initFilter
     | - bit 5: volIsCutoff (<187)
     | - bit 4: toFilter
     | - bit 3: noise on
     | - bit 2: pulse on
     | - bit 1: saw on
     | - bit 0: triangle on
  1  | flags 2
     | - bit 7: oscSync
     | - bit 6: ringMod
     | - bit 5: noTest
     | - bit 4: filterIsAbs
     | - bit 3: ch3off
     | - bit 2: band pass
     | - bit 1: high pass
     | - bit 0: low pass
  1  | attack/decay
     | - bit 4-7: attack
     | - bit 0-3: decay
  1  | sustain release
     | - bit 4-7: sustain
     | - bit 0-3: release
  2  | duty
  2  | cutoff/resonance
     | - bit 12-15: resonance
     | - bit 0-10: cutoff
  1  | upper nibble of resonance (for SID2) (>=199)
```

# Game Boy data (GB)

```
size | description
-----|------------------------------------
  1  | envelope params
     | - bit 5-7: length
     | - bit 4: direction
     | - bit 0-3: volume
  1  | sound length (64 = infinity)
  1  | flags
     | - bit 2: double wave width for GBA (>=196)
     | - bit 1: always init envelope
     | - bit 0: software envelope (zombie mode)
  1  | hardware sequence length
 ??? | hardware sequence (3*hwSeqLen bytes)
```

# OPL drums mode data (LD)

```
size | description
-----|------------------------------------
  1  | fixed frequency mode
  2  | kick freq
  2  | snare/hat freq
  2  | tom/top freq
```

# SNES data (SN)

```
size | description
-----|------------------------------------
  1  | attack/decay (bit 4-6: decay, bit 0-3: attack)
  1  | sustain/release (bit 5-7: sustain, bit 0-4: release)
  1  | flags (bit 4: envelope on, bit 0-2: gain mode)
  1  | gain
  1  | decay 2/sustain mode (>=131)
```

# Namco 163 data (N1)

```
size | description
-----|------------------------------------
  4  | waveform
  1  | wave pos
  1  | wave len
  1  | wave mode
     | **extra info** (>=164)
  1  | enable per channel wave pos/len
  8  | per channel wave pos (only if enabled)
  8  | per channel wave len (only if enabled)
```

# FDS/Virtual Boy data (FD)

```
size | description
-----|------------------------------------
  4  | mod speed
  4  | mod depth
  1  | init mod table with first wave
 1?? | modulation table (32 entries)
```

# old list of samples (SL) (<233)

```
  1  | number of samples
 1?? | sample indexes...
 4?? | pointers to samples...
```

# old list of wavetables (WL) (<233)

```
  1  | number of wavetables
 1?? | wavetable indexes...
 4?? | pointers to wavetables...
```

# new list of samples (LS) (>=233)

```
  2  | number of samples
 2?? | sample indexes...
 4?? | pointers to samples...
```

# new list of wavetables (LW) (>=233)

```
  2  | number of wavetables
 2?? | wavetable indexes...
 4?? | pointers to wavetables...
```

# MultiPCM data (MP)

```
  1  | attack rate
  1  | decay 1 rate
  1  | decay level
  1  | decay 2 rate
  1  | release rate
  1  | rate correction
  1  | LFO rate
  1  | vibrato depth
  1  | AM depth
  1  | flags (>=221)
```

# Sound Unit data (SU)

```
  1  | switch roles of phase reset timer and frequency
  1  | hardware sequence length (>=185)
 ??? | hardware sequence (5*hwSeqLen)
```

# ES5506 data (ES)

```
  1  | filter mode (0-3)
  2  | K1
  2  | K2
  2  | envelope count
  1  | left volume ramp
  1  | right volume ramp
  1  | K1 ramp
  1  | K2 ramp
  1  | K1 slow
  1  | K2 slow
```

# X1-010 data (X1)

```
  4  | bank slot
```

# NES DPCM sample map data (NE)

```
  1  | use sample map
 2?? | DPCM sample map (120 entries, only if enabled)
```

DPCM sample map entry:
```
  1  | pitch (0-15; otherwise no change)
  1  | delta counter value (0-127; otherwise no change)
```

# PowerNoise data (PN)

```
  1  | octave
```

# SID2 data (S2)

```
  1  | parameters
     | - bit 6-7: noise mode
     | - bit 4-5: wave mix mode
     | - bit 0-3: volume
```

# SID3 data (S3)

```
  1  | flags 1 (dutyIsAbs bit7, noise bit3, pulse bit2, saw bit1, tri bit0)
  1  | attack
  1  | decay
  1  | sustain
  1  | sustain rate
  1  | release
  1  | wave mix mode
  2  | duty
  1  | flags 2 (phaseMod bit7, specialWave bit6, 1bitNoise bit5, etc.)
  1  | phase modulation source channel
  1  | ring modulation source channel
  1  | hard sync source channel
  1  | special wave
  1  | flags 3 (left inversion bit1, right inversion bit0)
  1  | feedback
  1  | number of filters
 ??? | filter data (repeated per filter)
```

Filter entry:
```
  1  | parameters (enable bit7, init bit6, absCutoff bit5, etc.)
  1  | parameters 2 (scaleResOnce bit7)
  2  | filter cutoff
  1  | filter resonance
  1  | filter output volume
  1  | filter distortion level
  1  | filter mode (bandpass bit2, highpass bit1, lowpass bit0)
  1  | matrix connection (filter 4 bit3, 3 bit2, 2 bit1, 1 bit0)
  1  | cutoff scaling level
  1  | cutoff scaling center note
  1  | resonance scaling level
  1  | resonance scaling center note
```

# ESFM data (EF)

(not documented in detail here -- not relevant to PC Engine)
